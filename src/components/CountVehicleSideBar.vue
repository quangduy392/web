    ...
    >
        ...
        >
            <div class="p-2 text-center text-white border-bottom border-gray-500">
                ...
                >{{$t('vehicleCount.countEntry')}}: {{ totalEnter }}</h5>
                <h5
                    class="text-danger text-uppercase font-weight-bold"
                >{{$t('vehicleCount.countExit')}}: {{ totalLeave }}</h5>
            </div>
...
export default {
    ...
    mounted: function() {
        this.$nextTick(async function() {
            ...
            setTimeout(() => {
                ...
                if (
                    this.currentUser.selectedCarStatisticCameras &&
                    this.currentUser.selectedCarStatisticCameras.length
                ) {
                    const previoutSelectedCameras = this.currentUser.selectedCarStatisticCameras.filter(
                        camera => camera
                    );
    ...
    computed: {
        ...mapState({
            ...
            listCalculatedData: state => state.vehicleCount.listCalculatedData,
        ...
        zonesTableFields() {
            return [
                {
                    key: 'name',
                    label: this.$t('vehicleCount.manage.region')
                },
                {
                    thClass: 'text-right',
                    tdClass: 'number-people-col align-middle',
                    key: 'count',
                    label: this.$t('vehicleCount.count')
                }
            ];
        },
        totalEnter() {
    ...
    methods: {
        ...mapActions({
            ...
            saveSelectedCarStatisticCameras: 'auth/saveSelectedCarStatisticCameras'
        }),
        // Camera filter
        ...
        updateFilter() {
            ...
            this.saveSelectedCarStatisticCameras(selectedCameras);
