        ...
        >
            ...
            >
                <template slot="button-content">
                    <div class="text-white font-weight-bold'">{{$t('report.range')}}</div>
                    <div
                        class="text-muted font-weight-normal small"
                        v-line-clamp="1"
                    >{{ getTypeTreeSelectedText() }}</div>
                </template>
                <b-dropdown-form class="p-2">
                    <b-form-group>
                        ...
                        <VSelect
                            class="bg-white"
                            v-if="filter.typeSelected==='profile'"
                            v-model="filter.profileSelected"
                            label="name"
                            @input="updateFilter"
                            :options="listProfile"
                        />
                        <VSelect
                            class="bg-white"
                            v-if="filter.typeSelected==='group'"
                            v-model="filter.groupSelected"
                            @input="updateFilter"
                            :options="listGroup"
                        />
                    </b-form-group>
...
export default {
    ...
    computed: {
        typeOptions() {
            return [
                { text: this.$t('report.all'), value: 'all' },
                { text: this.$t('report.profile'), value: 'profile' },
                { text: this.$t('report.group'), value: 'group' }
            ];
        }
    },
    methods: {
        getTypeTreeSelectedText() {
            switch (this.filter.typeSelected) {
                case 'profile':
                    if (!this.filter.profileSelected) {
                        this.filter.profileSelected = this.listProfile[0];
                    }
                    return this.$t('report.profile') + ': ' + this.filter.profileSelected.name;
                case 'group':
                    if (!this.filter.groupSelected) this.filter.groupSelected = this.listGroup[0];
                    return this.$t('report.group') + ': ' + this.filter.groupSelected;
                default:
                    return this.$t('report.all');
            }
        },
        resetFilter() {
