...
const actions = {
        ...
        return Promise.all([
            ...
            const hasScheduledRegions = _.uniq(
                cameras
                    ...
                    .map(
                        camera =>
                            `${camera.district ? camera.district.trim() : 'default'}???${
                                camera.ward ? camera.ward.trim() : 'default'
                            }`
                    )
            );
            ...
            const reportGroupByArea = _.groupBy(
                allDayReport,
                region =>
                    `${region.district ? region.district.trim() : 'default'}???${
                        region.ward ? region.ward.trim() : 'default'
                    }`
            );
            ...
            for (const regionName in reportGroupByArea) {
                if (reportGroupByArea.hasOwnProperty(regionName)) {
                    if (hasScheduledRegions.indexOf(regionName) > -1) {
                        ...
                        const regionMorningReport = regionReportData.find(
                            region => typeof region.morningPresentTime !== 'undefined'
                        );
                        const regionAfternoonReport = regionReportData.find(
                            region => typeof region.afternoonPresentTime !== 'undefined'
                        );
    ...
    getReceptionCameras: context => {
        ...
        return Promise.all([
            ...
            console.log(receptionCameraStatuses);
