<template>
    <div class="animated fadeIn h-100">
        <sub-header hideBreadcrumb>
            <template slot="sub-header-left">
                <b-navbar-nav class="d-flex flex-row">
                    <b-nav-item to="/alert/event/newest">{{ $t('alert.event') }}</b-nav-item>
                    <b-nav-item to="/alert/event/bookmark">{{ $t('alert.bookmark') }}</b-nav-item>
                    <!-- <b-nav-item to="/alert/alert">{{ $t('alert.alert') }}</b-nav-item> -->
                </b-navbar-nav>
            </template>
        ...
        >
            ...
            <div v-if="selectedEvent.extra" style="position: absolute; top: 9px;">
                <h2 @click="$bvModal.hide('modal-video')" class="event-text-color">
                    {{ selectedEvent.text }}
                </h2>
            </div>
            <img
    ...
    methods: {
        ...
        changeVideo(event) {
            ...
            this.videoSrc = `${API_CONFIG.BASE_URL}event-image/${event.id}/${
                this.selectedEvent.type === 'reception' ? 'snapshot.jpg' : 'video.mp4'
            }`;
