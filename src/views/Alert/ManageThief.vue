<template>
    <div class="animated fadeIn h-100">
        <sub-header hideBreadcrumb>
            <template slot="sub-header-left">
                <b-navbar-nav class="d-flex flex-row">
                    <b-nav-item
                        to="/alert/manage/profile"
                        class="text-nowrap"
                    >{{ $t('alert.manage.profile') }}</b-nav-item>
                    <b-nav-item
                        to="/alert/manage/thief"
                        class="text-nowrap"
                    >{{ $t('alert.manage.thief') }}</b-nav-item>
                    <b-nav-item
                        to="/alert/manage/stranger"
                        class="text-nowrap"
                    >{{ $t('attendanceCommon.manage.stranger') }}</b-nav-item>
                    <b-nav-item
                        to="/alert/manage/region"
                        class="text-nowrap"
                    >{{ $t('alert.manage.region') }}</b-nav-item>
                    <b-nav-item
                        to="/alert/manage/monitor"
                        class="text-nowrap"
                    >{{ $t('alert.manage.monitor') }}</b-nav-item>
                </b-navbar-nav>
            </template>
        ...
        <b-modal
            ...
            :title="showDetectedImageTab ? $t('profile.updateDetectedImgTab') : $t('profile.updateProfile')"
        ...
        <b-modal
            ref="createModal"
            :size="showDetectedImageTab ? 'xl' : 'lg'"
            :title="showDetectedImageTab ? $t('profile.updateDetectedImgTab') : $t('profile.createProfile')"
...
export default {
    ...
    methods: {
        ...
        async openEditModal(profileId) {
            this.resetForm();
            this.resetDataImageDetected();
            await this.getProfile({ id: profileId, type: 'agency' });
        ...
        async handleOk(modal) {
            ...
            if (this.showDetectedImageTab) {
                await this.getImageProfile({ type: 'agency' });
        ...
        async submitForm() {
            if (this.form.id) {
                await this.updateOrCreate({ ...this.form, type: 'agency' });
                ...
                if (this.avatarImage !== null) {
                    this.uploadAvatar({ id: this.form.id, data: this.avatarImage }).then(() => {
                        this.getImageProfile({ type: 'agency' });
                    });
                }
            } else {
                await this.updateOrCreate({ ...this.form, type: 'agency' });
        ...
        async initData() {
            this.getImageProfile({ type: 'agency' });
            await this.getListProfiles({ type: 'agency' });
