    ...
    <div class="animated fadeIn h-100">
        <sub-header hideBreadcrumb>
            <template slot="sub-header-left">
                <b-navbar-nav class="d-flex flex-row">
                    <b-nav-item to="/process-monitor/manage/region" class="text-nowrap">{{
                        $t('processMonitor.manage.region')
                    }}</b-nav-item>
                    <b-nav-item to="/process-monitor/manage/monitor" class="text-nowrap">{{
                        $t('processMonitor.manage.monitor')
                    }}</b-nav-item>
                </b-navbar-nav>
            </template>
                    ...
                    <div class="d-flex align-items-center text-nowrap">
                        <b-button variant="outline-primary" @click="openCreateModal">
                            ...
                            <span class="text-uppercase">{{
                                $t('processMonitor.zone.createZone')
                            }}</span>
                        </b-button>
                    </div>
                </div>
            ...
            >
                ...
                <template slot="empty">
                    <div class="text-center">
                        <span>{{ $t('processMonitor.zone.empty') }}</span>
                    </div>
                </template>
            </b-table>
        ...
        <b-modal
            ...
            :title="
                `${
                    zoneInfo.id
                        ? $t('processMonitor.zone.updateZone')
                        : $t('processMonitor.zone.createZone')
                }`
            "
        ...
        >
            <b-form-group :label="$t('processMonitor.zone.zoneName')">
                <b-form-input
                    maxlength="255"
                    v-model="zoneInfo.name"
                    v-validate="'required'"
                    :placeholder="$t('processMonitor.zone.zoneName')"
                    name="zoneName"
                    :data-vv-as="$t('processMonitor.zone.zoneName')"
                ></b-form-input>
        ...
        <b-modal
            ...
            ok-variant="danger"
            @ok="onDeleteZone"
            >{{ $t('processMonitor.zone.deleteConfirmMsg') }}</b-modal
        >
    </div>
</template>
...
export default {
    ...
    methods: {
            ...
            this.$validator.validateAll().then(result => {
                if (result) {
                    ...
                    this.$toaster.success(
                        ...
                        } ${this.$i18n.t('processMonitor.zone.zoneSuccess')}`
                    );
                }
            });
        },
        ...
        async onDeleteZone(modal) {
            ...
            this.$toaster.success(this.$i18n.t('processMonitor.zone.deleteZoneMess'));
        },
        openCreateModal() {
            this.resetForm();
            this.$refs.createOrUpdateZoneModal.show();
